...
**Domain:** Locked-down Windows Electron kiosk wrapping a third-party React/MUI SaaS (Magicline) with persistent CSS/JS injection, USB HID badge input, DPAPI-encrypted credentials, and GitHub Releases auto-update.
**Researched:** 2026-04-08
**Confidence:** HIGH (core runtime, builder, updater, storage verified against current npm + official docs); MEDIUM (optional overlay bundler — intentionally minimal).

---

## TL;DR — Prescriptive Stack

Plain Electron + HTML/CSS/JS, packaged with `electron-builder` as NSIS, updated via `electron-updater` against GitHub Releases, credentials in Electron `safeStorage` (DPAPI-backed), non-secret config in `electron-store`, logs in `electron-log`. **No bundler. No renderer framework. No native modules.** This is a single-device, single-window shell — complexity is the enemy.
...
```bash
# Project bootstrap
mkdir bee-strong-pos-kiosk && cd bee-strong-pos-kiosk
npm init -y

# Runtime (all are dependencies — bundled into the app package)
npm install electron-updater@^6.8.3 electron-log@^5.2 electron-store@^10.1

# Electron itself + packager are devDependencies
npm install --save-dev electron@^41.1.1 electron-builder@^26.8.1
```

**Do NOT install:** `keytar`, `node-keytar`, `node-hid`, `node-global-key-listener`, `serialport`, `robotjs`, `webpack`, `vite`, `react`, `vue`, `typescript` (initial scope). Each would add a native toolchain or a build step for zero value in a single-device project.
...
```jsonc
{
  ...
  "build": {
    ...
    "publish": [
      { "provider": "github", "owner": "<your-gh-user>", "repo": "bsfpos" }
    ]
  }
}
```
...
**If `safeStorage.isEncryptionAvailable()` returns `false` at first run:**
...
**If Magicline deploys a breaking MUI `css-xxxxx` class change:**
...
**If overlays grow past ~5 screens or need shared state:**
...
**If the repo must become private:**
...
---
*Stack research for: Windows Electron kiosk wrapping Magicline SaaS*
*Researched: 2026-04-08*
