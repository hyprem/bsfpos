...
- **D-14:** Confirmed SKU is **Windows 11 Pro** (resolved 2026-04-08 during planning). Assigned Access is UWP-only (incompatible with Electron Win32 app); Shell Launcher v2 is Enterprise/Education/IoT-only (not available on Pro). Chosen mechanism: **custom HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon\\Shell registry fallback** pointing at the kiosk account, combined with GPO/local policy hardening via .reg + PowerShell scripts. Runbook must include a "re-apply after Windows feature update" verification step because Windows Update has been reported to reset this key.
- **D-15:** Auto-login uses classic **AutoAdminLogon** (`HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon\\DefaultUserName` + `DefaultPassword` + `AutoAdminLogon=1`). Accepted tradeoff: the kiosk account's password is stored plaintext in the registry. Mitigations documented in runbook: (a) kiosk account has zero rights beyond running the POS app and no access to any sensitive resource, (b) separate strong local-admin account for maintenance, (c) device is physically in a staffed gym, (d) BitLocker recommended in runbook for disk-at-rest protection. Magicline credentials are a separate concern handled by Phase 3 via Electron `safeStorage` (DPAPI) — they are NEVER stored as the kiosk Windows account password.
