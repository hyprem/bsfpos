...
human_verification:
  ...
  - test: "Auto-update + safe window + rollback (P5-15..P5-24)"
    ...
    why_human: "Requires publishing real GitHub Releases, waiting for the 09:00–12:00 maintenance window or triggering an idle-expiry reset, and exercising the 2-minute health watchdog against a deliberately broken build."
...
| 05-03 update-gate+session-hook | `onPostReset(cb)` fires once after successful hardReset, NOT on suppressed/loop-active; `onUpdateDownloaded` arms maintenance interval + single-shot listener; first trigger wins; `isMaintenanceWindow` true only for hours 9,10,11 (flipped from 3,4 in 0.1.3 quick 260414-iiv) | VERIFIED — `sessionReset.js:54,170,209`, `updateGate.js`, 11/11 unit tests green |
...
| ADMIN-07 | quitAndInstall gated behind safe window (post-reset or 09:00–12:00) | 05-03, 05-04 | VERIFIED | `updateGate.js` `isMaintenanceWindow` hours 9/10/11 (flipped from 3/4 in 0.1.3); `onPostReset` hook; first-trigger-wins; `main.js:110` wires gate |
...
| P5-20 | Alternatively wait until 09:00–12:00 window after a downloaded update | `update.install` with `trigger=maintenance-window` logs; new version boots | [ ] |
